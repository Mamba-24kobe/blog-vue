<template>
  <div>
    <m-top-handle title="昵称" content="新建用户" path="/users/editor/new">
...
export default {
  ...
  methods: {
    async getList() {
      let res = await this.$axios.get("/rest/users");
    ...
    deleteHandle(data) {
      this.$confirm(`是否确定要删除留言: ${data.content} ?`, "提示", {
        ...
        .then(async () => {
          await this.$axios.delete(`/rest/users/${data._id}`);
          this.getList();
          this.$message({
            type: "success",
            message: "删除成功!"
    ...
    handleSelectionChange(val) {
      console.log(val)
      this.multipleSelection = val.map(v => { return {email: v.email, subject: v.nickName} } )
