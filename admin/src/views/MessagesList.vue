  <template>
  <div>
    <m-top-handle title="昵称" content="留言" path="/messages/editor/new"></m-top-handle>
...
export default {
  ...
  methods: {
    async getList() {
      let res = await this.$axios.get("/rest/messages");
    ...
    deleteHandle(data) {
      this.$confirm(`是否确定要删除留言: ${data.content} ?`, "提示", {
        ...
        .then(async () => {
          await this.$axios.delete(`/rest/messages/${data._id}`);
          this.getList();
          this.$message({
            type: "success",
            message: "删除成功!"
