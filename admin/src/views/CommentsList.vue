<template>
  <div>
    <m-top-handle title="昵称" content="评论" path="/comments/editor/new"></m-top-handle>
...
export default {
  ...
  methods: {
    async getList() {
      let res = await this.$axios.get("/rest/comments");
    ...
    deleteHandle(data) {
      this.$confirm(`是否确定要删除留言: ${data.content} ?`, "提示", {
        ...
        .then(async () => {
          await this.$axios.delete(`/rest/comments/${data._id}`);
          this.getList();
          this.$message({
            type: "success",
            message: "删除成功!"
